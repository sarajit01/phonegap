...
class AngularGenerate extends ThemeBuilder {
	...
	public static $nodePlugins = array(
		'dependencies'    => array(
			...
			'classlist.js' => '^1.1.20150312', '@ng-bootstrap/ng-bootstrap' => '^1.0.0-alpha.29', 'primeng' => '^4.1.2',
		),
...
EOD;
	...
	public static function appendFile( $filename, $insert, $after = '' ) {
		$newline        = '';
		$insertPosition = 0;  // variable for saving Routes position
...
EOD;
	...
	public static function createUnwrapDirective() {
		...
		//self::moduleDeclare( '_directives', 'unwrap-tag.directive', $directive->name . 'Directive' );
	...
	public static function createHrefDirective() {
		...
		//self::moduleDeclare( '_directives', 'href-prevent-default.directive', $directive->name . 'Directive' );
	...
	public static function extraModules() {
		...
		self::copy( __DIR__ . '/templates/angular/theme', $path );
	...
	public static function layoutComponent( $output, $param ) {
		...
		$component->const    = array( 'declare let mApp: any;' );
		...
		$themeUrl = '/' . substr( self::$themePath, 0, -1 );
...
EOD;
	...
	private static function createModule( $path, $param ) {
		$selector = implode( '-', array_slice( explode( '/', $param[ 'page' ] ), -2, 2 ) );
	...
	public static function updateUrl( $output ) {
		if ( ! empty( $output ) ) {
			...
			$output = preg_replace_callback( '/<li.*?(class="m-menu__item.*?").*?>/', function ( $match ) use ( &$put ) {
				if ( strpos( $match[ 0 ], 'data-redirect="true"' ) === false && strpos( $match[ 0 ], 'm-menu__item--submenu' ) === false ) {
					$match[ 0 ] = str_replace( $match[ 1 ], $match[ 1 ] . ' routerLinkActive="m-menu__item--active" routerLinkActiveOptions="{ exact: true }" ', $match[ 0 ] );
				}
				if ( strpos( $match[ 0 ], 'm-menu__item--rel' ) !== false && ! $put ) {
					$match[ 0 ] = str_replace( $match[ 1 ], $match[ 1 ] . ' routerLinkActive="m-menu__item--active" routerLinkActiveOptions="{ exact: true }" ', $match[ 0 ] );
					$put        = true;
				}
				return $match[ 0 ];
